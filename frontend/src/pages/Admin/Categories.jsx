...
import { FaLayerGroup, FaChevronLeft, FaPlus, FaSearch, FaEdit, FaTrash, FaTimes } from "react-icons/fa";
...
export default function Categories() {
    ...
    const [editCategoryData, setEditCategoryData] = useState(null); // holds { id, categoryName }
    ...
    const handleEditSubmit = async () => {
        if (!editCategoryName.trim() || !editCategoryData) return;
        try {
            await updateCategory(editCategoryData.id, editCategoryName);
        ...
        <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-800 pb-16 selection:bg-indigo-100 relative">
                ...
                {isAddModalOpen && (
                    ...
                    >
                        ...
                        >
                            ...
                            <h2 className="text-2xl font-bold text-slate-900 mb-6">Add New Category</h2>

                            <div className="mb-8">
                                <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-2">Category Name</label>
                                <input
                                    type="text"
                            ...
                            <div className="flex gap-3 justify-end">
                                ...
                                <button onClick={handleAddSubmit} className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold transition-all shadow-sm">Save Category</button>
                            </div>
                        </motion.div>
                    </motion.div>
                )}
                ...
                {editCategoryData && (
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl relative"
                        >
                            <button onClick={() => setEditCategoryData(null)} className="absolute top-6 right-6 text-slate-400 hover:text-slate-800 transition-colors">
                                <FaTimes size={20} />
                            </button>
                            ...
                            <div className="flex gap-3 justify-end">
                                <button onClick={() => setEditCategoryData(null)} className="px-5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold transition-all">Cancel</button>
                                <button onClick={handleEditSubmit} className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-semibold transition-all shadow-sm">Update Category</button>
                            </div>
                        </motion.div>
                    </motion.div>
                )}
                ...
                {deleteCategoryId && (
                    ...
                    >
                        ...
                        >
                            ...
                            <p className="text-slate-500 text-sm mb-8">Are you sure? This action cannot be undone and may affect books linked to this category.</p>

                            <div className="flex gap-3 justify-center">
                                <button onClick={() => setDeleteCategoryId(null)} className="px-5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold transition-all w-full">Cancel</button>
            ...
            <motion.div variants={container} initial="hidden" animate="show" className="max-w-7xl mx-auto px-4 sm:px-8">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 gap-4">
                    <div>
                        <h1 className="text-3xl sm:text-4xl font-light tracking-tight text-slate-900">Categories</h1>
                        <p className="text-slate-500 mt-2 font-medium">Manage taxonomy and library classifications.</p>
                    </div>
                    <button
                ...
                <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {filteredCategories.map((cat, idx) => (
                        <motion.div variants={item} key={cat.id || idx} className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm hover:shadow-md hover:border-indigo-100 transition-all group flex flex-col justify-between h-48">
                            ...
                            <div>
                                <h2 className="text-lg font-bold text-slate-900 leading-tight mb-1">{cat.categoryName}</h2>
